...
std::vector<long double> quad_points(long double a, long double b, int k){
    std::vector<long double> _xs;
    ...
    std::transform(_xs.begin(), _xs.end(), std::back_inserter(xs), [a, b](long double x) -> long double{return 0.5 * (a * (1 - x) + b * (x + 1));});
    return xs;
};

std::vector<long double> quad_weights(long double a, long double b, int k){
    std::vector<long double> ws;
    
    if(k == 1)
        ws = std::vector<long double>{2.};
    else if(k == 2)
        ws = std::vector<long double>{1., 1.};
    else if(k == 3)
        ws = std::vector<long double>{5./9.,
            8./9.,
            5./9.};
    else if(k == 4)
        ws = std::vector<long double>{(18.-std::sqrt(30.))/36.,
            (18.+std::sqrt(30.))/36.,
            (18.+std::sqrt(30.))/36.,
            (18.-std::sqrt(30.))/36.};
    else if(k == 5)
        ws = std::vector<long double>{(322-13.0*std::sqrt(70))/900,
...
long double compute_spline(std::vector<long double>& knot, int p, int nr, long double x){
    auto a = knot[nr];
    auto b = knot[nr+p];
    auto c = knot[nr+1];
    auto d = knot[nr+p+1];
...
int compute_p(std::vector<long double>& knot){
    int p = 0;
    auto initial = knot[p];
...
int number_of_elements(std::vector<long double>& knot){
    int n = 0;
    auto initial = knot[n];
...
int number_of_dofs(std::vector<long double>& knot, int p){
    return knot.size() - p - 1;
}

std::tuple<long double, long double> element_boundry(std::vector<long double>& knot, int p, int nr){
    auto initial = knot[0];
    int k = 0;
    long double low = 0;
    long double high = 0;
...
int first_dof_on_element(std::vector<long double>& knot, int p, int nr){
    auto [l, h] = element_boundry(knot, p, nr);
    return (knot.rend() - std::find(knot.rbegin(), knot.rend(), l) - 1) - p;
}

std::tuple<int, int> dofs_on_element(std::vector<long double>& knot, int p, int nr){
...
Matrix gen(int riga, int nxx, int pxx, int rxx){
	...
	if(riga == 0){
			...
			std::transform(_knot.begin(), _knot.end(), std::back_inserter(knot), [nxx](long double x) -> long double{return x/nxx;});
	}else if(riga == 1){
			for(int i=0; i<=pxx; i++)
					knot.push_back(0);
